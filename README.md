...
`lite` converts the server-derived city, interior, exit, and tutorial map set;
`full` converts every map. Both modes generate the complete sprite, effect,
palette, icon, audio, localization, packet, item, skill, status, quest, and
collision data needed by the client. `--skip-assets` refreshes the rAthena and
collision outputs without reconverting client art.
