"""Run the client's compiled Lua 5.1 tables (``.lub``) straight from a GRF.

Many client tables are Lua rather than text: status icons, skill info, job
names. They were compiled by a 32-bit Lua 5.1 (size_t = 4 bytes), and a 64-bit
host Lua refuses that header. Lua 5.1 bytecode only uses size_t for string
length prefixes, so rebasing a chunk to the host layout is a structural rewrite
(``rebase_chunk``), with no interpretation of the code itself.

Executing the tables needs ``lupa`` (``pip install ragx[lua]``); rebasing does
not.
"""
...
LUA_FILES = "data\\luafiles514\\lua files\\"
...
def rebase_chunk(data: bytes, dst_size_t: int = 8) -> bytes:
    ...
    Plain-text Lua passes through unchanged; any other header is refused rather
    than guessed at.
    """
...
class LuaEnv:
    """A Lua 5.1 state that loads ``.lub`` files straight from a GRF archive."""

    def __init__(self, grf):
        try:
            import lupa.lua51 as lua51
        except ImportError as exc:  # pragma: no cover - depends on the install
            raise SystemExit("this command runs the client's Lua tables and needs "
                             "lupa: pip install 'ragx[lua]'") from exc
        self.grf = grf
        # encoding=None: Lua strings come back as bytes; client tables are CP949,
        # so decode at the call site with decode().
    ...
    def load(self, lua_path: str, optional: bool = False, data_root: str = "data") -> bool:
        """Execute one table, e.g. ``stateicon/efstids``.

        ``lua_path`` is relative to ``Lua Files``. Localised clients mirror that
        tree under roots such as ``data\\english``; ``data_root`` picks one.
        """
        full = LUA_FILES.replace("data\\", data_root + "\\", 1) \
            + lua_path.replace("/", "\\") + ".lub"
        if full not in self.grf:
            if optional:
                return False
            raise FileNotFoundError(full)
        chunk = rebase_chunk(self.grf.read(full), self._host_size_t)
        result = self._loadstring(chunk, b"@" + lua_path.encode())
        if isinstance(result, tuple):  # (nil, error message)
            raise ValueError(f"{lua_path}: {result[1]}")
        result()
        return True

    def table(self, name: str):
        """A global value (usually a table), or None."""
