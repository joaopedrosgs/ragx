"""Export every STR effect to a texture atlas PNG + keyframe JSON.

For each ``data\\texture\\effect\\**\\<name>.str`` in the client GRF:

  effects/<rel path>.png    every bitmap the effect references, packed
  effects/<rel path>.json   fps + per-layer keyframe tracks + atlas rects

We export the *keyframes*, not baked per-frame data: the client's morph
model (a type-0 "basis" keyframe plus type-1 keyframes holding per-frame
increments) is a cheap linear interpolation the Godot runtime redoes at
playback time (see actors/str_effect.gd). That keeps the JSON tiny
(~130 keyframes/effect) and the animation frame-rate independent.

JSON layout (consumed by the Godot EffectDb / StrEffect runtime):
  ...
  blend is a resolved BLEND_* family (0 mix, 1 add, 2 premul); inherited
  (0,0) factors are already carried forward here so the runtime needs no
  blend history.

Usage:
    python -m ragnadot.effect_export --all --out C:/Users/pedro/Documents/ragnarok
    python -m ragnadot.effect_export lord stormgust --out ...
"""
...
def _decode_texture(data: bytes) -> np.ndarray | None:
    ...
    Effect BMPs are 24-bit (no alpha): under the additive/premultiplied
    blend modes that dominate effects, black contributes nothing, so a
    solid 255 alpha is correct. TGAs keep their own alpha channel."""
    ...
    # Magic pink (255, 0, 255) is RO's transparency key; quantise the top
    # nibble like the client before testing so near-pink also keys out.
...
def _round(value: float, ndigits: int = 3) -> float:
    """JSON-safe round, including Gravity's NaN/Infinity delay sentinels."""
...
def export_one(effect, folder: str, grf: GrfStack, out_root: Path) -> str:
    from PIL import Image
    ...
    # A texture "has alpha" if it carries real transparency (TGA channel or
    # a magic-pink key); opaque BMPs do not. Effect BMPs are meant to blend
    # additively (black adds nothing), so a layer whose textures are all
    # opaque must never use alpha compositing or it shows as a solid block.
...
def main() -> None:
    ...
    parser.add_argument("effects", nargs="*",
                        help="effect names relative to texture/effect/, no ext")
    ...
    if args.all:
        ...
        # also the few effects living under data\sprite (jong_mini etc.)
    else:
        targets = []
        ...
        for n in names:
            ...
            if rel in wanted or rel.rsplit("\\", 1)[-1] in wanted:
                targets.append(n)
        if not targets:
            parser.error("no matching effects (try --all or check the name)")

    out_root = Path(args.out) / "effects"
    out_root.mkdir(parents=True, exist_ok=True)
    (out_root / ".gdignore").write_text("", encoding="utf-8")
    ...
    for done, name in enumerate(targets, 1):
        try:
            ...
            if status == "ok":
                exported.append(
                    name[len(EFFECT_PREFIX):-4].replace("\\", "/"))
    ...
    if args.all:
        # Keep the exact corpus separate from any older/custom loose effects
        # already in the generated tree. Scene baking and count reporting read
        # this instead of mistaking leftovers from another client for KRO data.
        (out_root / ".kro-str-manifest").write_text(
            json.dumps({
                "version": 1,
                "effects": sorted(exported),
